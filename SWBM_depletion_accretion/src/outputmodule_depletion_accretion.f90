...
!  end subroutine  write_MODFLOW_WEL
  ...
  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  subroutine write_MODFLOW_WEL(im,imonth,total_n_wells,n_wel_param,WELL_LAY,WELL_ROW,WELL_COL,PMP_RCH_RATE)
  
  INTEGER             :: im, imonth, total_n_wells, n_wel_param, iwell
  INTEGER, INTENT(in) :: WELL_LAY, WELL_ROW, WELL_COL
  REAL, INTENT(in)    :: PMP_RCH_RATE
  
  open(unit=536, file='SVIHM.wel',Access = 'append', status='old')
  	if (imonth == 1 .or. imonth == 2 .or. imonth == 3 .or. imonth == 4 .or. &                              ! If October-March
  	    imonth == 5 .or. imonth == 6) then
  	  write(536,'(I10,I10,A28,I4)')total_n_wells+1, n_wel_param-2, '               Stress Period',im         ! Only MFR is active, subtract number of ditches represented 
  	else if (imonth == 7 .or. imonth == 8) then                                                            ! If April-May
  	  write(536,'(I10,I10,A28,I4)')total_n_wells+1, n_wel_param, '               Stress Period',im           ! MFR and Ditches are active, use all WEL parameters
  	else if (imonth == 9 .or. imonth == 10) then                                                           ! If June - July
  	  write(536,'(I10,I10,A28,I4)')total_n_wells+1, n_wel_param-7, '               Stress Period',im         ! Only Ditches are active, subtract number of MFR segments represented
  	else if (imonth == 11 .or. imonth == 0) then                                                           ! If August-September
  	  write(536,'(I10,I10,A28,I4)')total_n_wells+1, n_wel_param-9, '               Stress Period',im         ! Only Ditches are active, subtract number of MFR and Ditch segments represented                                              
  	end if  
  ...
  if (PMP_RCH_RATE .LT. 0) then                                                       ! Streamflow Depletion
    if (imonth == 7) then                                                             ! If April apply half the pumping rate since it doesn't start until the 15th
      write(536,'(3I10,ES15.3)') WELL_LAY, WELL_ROW, WELL_COL, PMP_RCH_RATE/2       
      write(*,'(a22,f8.2,a15,i3,a6,i3)')'Additional pumping of ',PMP_RCH_RATE/2, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
      write(800,'(a22,f8.2,a15,i3,a6,i3)')'Additional pumping of ',PMP_RCH_RATE/2, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
    else if (imonth .GE. 8 .and. imonth .LE. 11) then                                 ! If May - August apply full pumping rate
    	write(536,'(3I10,ES15.3)') WELL_LAY, WELL_ROW, WELL_COL, PMP_RCH_RATE       
      write(*,'(a22,f8.2,a15,i3,a6,i3)')'Additional pumping of ',PMP_RCH_RATE, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
      write(800,'(a22,f8.2,a15,i3,a6,i3)')'Additional pumping of ',PMP_RCH_RATE, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
    else                                                                              ! If September - March no additional pumping
    	write(536,'(3I10,ES15.3)') WELL_LAY, WELL_ROW, WELL_COL, 0.0
    end if
  else if (PMP_RCH_RATE .GT. 0) then                                                 ! Streamflow Accretion  
  	if (imonth .GE. 4 .and. imonth .LE. 9) then                                       ! If Jan-June recharge is active
  		write(536,'(3I10,ES15.3)') WELL_LAY, WELL_ROW, WELL_COL, PMP_RCH_RATE       
      write(*,'(a22,f8.2,a15,i3,a6,i3)')'Additional recharge of ',PMP_RCH_RATE, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
      write(800,'(a22,f8.2,a15,i3,a6,i3)')'Additional recharge of ',PMP_RCH_RATE, &
          ' added to row: ', WELL_ROW, ' col: ',WELL_COL
    else
    	 write(536,'(3I10,ES15.3)') WELL_LAY, WELL_ROW, WELL_COL, 0.0
    end if
  else
  	write(536,'(3I10,ES15.3)') 1, 1, 1, 0.0	                                         ! Place dummy well outside of domain
  end if
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    subroutine write_MODFLOW_RCH(im,imonth,nday,nrows,ncols,output_zone_matrix)  
      ...
      write(84,*)'1  0'
      write(84,'(a64)')'        18   1.00000(210e14.6)                   -1     RECHARGE'
      ...
      write(84,'(210e14.6)') recharge_matrix	  
      ttl_rch = sum(recharge_matrix*10000.0)
      write(900,*) ttl_rch, ttl_rch*nday(imonth)/1E6
