...
!   open(unit=887,file='precip_m_Replacement_July2017.txt')     ! Missing data replaced with value at other station (CAL and FJN)  
   ...
   CALL EXECUTE_COMMAND_LINE('cp SVIHMv3.1_ETS_template.txt SVIHMv3.1.ets')
   CALL EXECUTE_COMMAND_LINE('cp SVIHMv3.1_SFR_template.txt SVIHMv3.1.sfr')
   CALL EXECUTE_COMMAND_LINE('cp SVIHMv3.1_WEL_template.txt SVIHMv3.1.wel')
   CALL EXECUTE_COMMAND_LINE('cp UCODE_SFR_template_v3.1.txt SVIHMv3.1_SFR.jtf')
   ...
   do im=1, nmonth            ! Loop over each month
     ...
     do jday=1, nday(imonth)           ! Loop over days in each month
       ...
       if (precip .GE. (0.2*ref_et)) then
         ...
         call IRRIGATION_ILR(ip, imonth, jday, precip_adjusted)    
