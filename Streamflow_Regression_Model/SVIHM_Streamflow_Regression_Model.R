...
preproflow = function(daily_means_all, Max_Num_Missing_Days, Streams) {
...
tribs_data_post_WY1972 = function(monthly_cleaned_all, Streams, Post_WY1972) {
  #initialize empty output list
...
write_svihm_tables = function(all_tribs_regress_pre_WY1973, all_tribs_regress_post_WY1972, Streams){
  #Pre-WY1973
...
#.#########################################################.#
###########        Generate streamflow_input.txt     ########
#.#########################################################.#

generate_streamflow_input_txt = function(end_date = as.Date("2018/9/30")){
  
  # rm(list=ls())
  ### 0. Specify end date of model and generate date vectors
  ...
  NumDays = as.numeric(diff(Months_plus_one))                                  #Number of days in each simulation month
  ...
  all_tribs_regress_pre_WY1973 = tribs_regress_pre_WY1973(monthly_cleaned_all = monthly_cleaned_all, 
                                                          Streams = Streams, Pre_WY1973=Pre_WY1973)
  ...
  all_tribs_regress_post_WY1972 = tribs_data_post_WY1972(monthly_cleaned_all = monthly_cleaned_all,
                                                         Streams = Streams, Post_WY1972=Post_WY1972)
  
  ### 5. Write SVIHM coefficient tables
  coef_tables = write_svihm_tables(all_tribs_regress_pre_WY1973 = all_tribs_regress_pre_WY1973, 
  ...
  model_period_flows = populate_SVIHM_flows(monthly_cleaned_all = monthly_cleaned_all, 
                                            ...
                                            Streams=Streams, SVIHM_months = SVIHM_months, NumDays = NumDays)
  ...
  for(i in 1:length(model_period_flows)){
    ...
    colnames_new = c(colnames(WY1991_2018_Streamflow),paste0(Streams[i+1],"_Avg_Flow_m3day"))
    
    WY1991_2018_Streamflow = cbind(WY1991_2018_Streamflow, trib_model_period$avg_flow_m3day)
    colnames(WY1991_2018_Streamflow) = colnames_new
  }
  #Insert records for two ungaged columns
  WY1991_2018_Streamflow = cbind(WY1991_2018_Streamflow, johnson_crystal)
