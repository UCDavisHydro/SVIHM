...
#output_type is a string for specfying output data type (volume or rate)
MODFLOW_Budget = function(filename, mf_bud_terms, output_type, suffix) {
  library(magrittr)
  ...
  MODFLOW_Fluxes = data.frame(Month = format(seq(as.Date('1990/10/1'), as.Date('2011/9/30'), by = 'month'),'%b-%Y'),
                                      Water_Year = rep(seq(1991,2011),each=12),
                                      STORAGE_in_m3day = STORAGE_TS_Flux_in,
                                      STORAGE_out_m3day = STORAGE_TS_Flux_out,
                                      STORAGE_net_m3day = STORAGE_TS_Flux_net,
                                      CONSTANT_HEAD_in_m3day = CONSTANT_HEAD_TS_Flux_in,
                                      CONSTANT_HEAD_out_m3day = CONSTANT_HEAD_TS_Flux_out,
                                      CONSTANT_HEAD_net_m3day = CONSTANT_HEAD_TS_Flux_net,
                                      WELLS_in_m3day = WELLS_TS_Flux_in,
                                      WELLS_out_m3day = WELLS_TS_Flux_out,
                                      WELLS_net_m3day = WELLS_TS_Flux_net,
                                      RECHARGE_in_m3day = RECHARGE_TS_Flux_in,
                                      RECHARGE_out_m3day = RECHARGE_TS_Flux_out,
                                      RECHARGE_net_m3day = RECHARGE_TS_Flux_net,
                                      ET_SEGMENTS_in_m3day = ET_SEGMENTS_TS_Flux_in,
                                      ET_SEGMENTS_out_m3day = ET_SEGMENTS_TS_Flux_out,
                                      ET_SEGMENTS_net_m3day = ET_SEGMENTS_TS_Flux_net,
                                      STREAM_LEAKAGE_in_m3day = STREAM_LEAKAGE_TS_Flux_in,
                                      STREAM_LEAKAGE_out_m3day = STREAM_LEAKAGE_TS_Flux_out,
                                      STREAM_LEAKAGE_net_m3day = STREAM_LEAKAGE_TS_Flux_net,
                                      DRAINS_in_m3day = DRAINS_TS_Flux_in,
                                      DRAINS_out_m3day = DRAINS_TS_Flux_out,
                                      DRAINS_net_m3day = DRAINS_TS_Flux_net,
                                      TOTAL_in_m3day = TOTAL_TS_Flux_in,
                                      TOTAL_out_m3day = TOTAL_TS_Flux_out,
                                      TOTAL_net_m3day = TOTAL_TS_Flux_net,
                                      Error_Cumulative_percent = Cumulative_Mass_Balance_percent_diff,
                                      Error_Stress_Period_percent = SP_Mass_Balance_percent_diff,
                                      Error_Timestep_percent = TS_Mass_Balance_percent_diff)
  if (missing(suffix)) {
    ...
    } else if (output_type == 'rate') {
      return(MODFLOW_Fluxes)
    } else {
      stop('Unknown entry for output_type')
    }
  } else {
