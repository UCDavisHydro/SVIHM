...
! Water source dry or sub or n* (555) with any irrig type -> et/noIRR
  ...
  INTEGER  :: DEP_ACR_FLAG, WELL_LAY, WELL_ROW, WELL_COL        ! Flag for depletion/accretion, location of cells for streamflow depletion/accretion
  REAL     :: WELL_OUT_RATE, RCH_IN_RATE                       ! Rate for pumping and recharge
  ...
  open(unit=10, file ='depletion_accretion_rates.txt', status ='old')
  read(10,*) DEP_ACR_FLAG
  read(10,*) WELL_OUT_RATE, RCH_IN_RATE
  read(10,*) WELL_LAY, WELL_ROW, WELL_COL
  close(10)
  
  if (DEP_ACR_FLAG==-1) then
    write(*,*) 'Streamflow depletion analysis is active'
    write(*,'(A15, F8.2, A8)') 'Pumping Rate = ',WELL_OUT_RATE, ' m^3/day'
    write(*,'(A16,I1,A5,I3,A5,I3)') 'Location: Layer ',WELL_LAY, ' Row ', WELL_ROW, 'Col ', WELL_COL
    write(800,*) 'Streamflow depletion analysis is active'
    write(800,'(A15, F8.2, A8)') 'Pumping Rate = ',WELL_OUT_RATE, ' m^3/day'
    write(800,'(A16,I1,A5,I3,A5,I3)') 'Location: Layer ',WELL_LAY, ' Row ', WELL_ROW, 'Col ', WELL_COL
  else if (DEP_ACR_FLAG==1) then
  	write(*,*) 'Streamflow accretion analysis is active'
    write(*,'(A15, F8.2, A8)') 'Recharge Rate = ',RCH_IN_RATE, ' m^3/day'
    write(*,'(A16,I1,A5,I3,A5,I3)') 'Location: Layer ',WELL_LAY, ' Row ', WELL_ROW, 'Col ', WELL_COL
    write(800,*) 'Streamflow depletion analysis is active'
    write(800,'(A15, F8.2, A8)') 'Recharge Rate = ',RCH_IN_RATE, ' m^3/day'
    write(800,'(A16,I1,A5,I3,A5,I3)') 'Location: Layer ',WELL_LAY, ' Row ', WELL_ROW, 'Col ', WELL_COL
  else
    write(*,*)'depletion/accretion flag must be -1 or 1 in depletion_accretion_rates.txt'
    write(800,*)'depletion/accretion flag must be -1 or 1 in depletion_accretion_rates.txt'
    call EXIT
  end if
...
!  open(unit=887,file='precip_m_Replacement_July2017.txt')     ! Missing data replaced with value at other station (CAL and FJN)  
  ...
  write(900,*)'Total_Recharge_m^3/day  Total_Recharge_Mm^3/Month'                    
   ...
   do im=1, nmonth            ! Loop over each month
     ...
     do jday=1, nday(imonth)                         ! Loop over days in each month
       ...
       call monthly_pumping(im, jday, total_n_wells)
		   ...
		   call write_MODFLOW_RCH(im,imonth,nday,nrows,ncols,output_zone_matrix,DEP_ACR_FLAG, WELL_ROW, WELL_COL, RCH_IN_RATE) 
       ...
       if (DEP_ACR_FLAG==-1) then      ! If -1, depletion is active
         call write_MODFLOW_WEL_depletion(im,imonth,total_n_wells,n_wel_param,WELL_LAY, WELL_ROW, WELL_COL,WELL_OUT_RATE)    
       else if (DEP_ACR_FLAG==1) then  ! If 1, accretion is active
