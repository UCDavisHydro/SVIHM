...
# file.copy(copy_these_files, SWBM_file_dir, recursive = FALSE, copy.mode = TRUE)
# Odd error - LS fix
if (!dir.exists(SWBM_file_dir)) {
  # Create directories
  dir.create(SWBM_file_dir, recursive = TRUE)
}
for (curfile in copy_these_files) {
  file.copy(curfile, paste0(SWBM_file_dir,'/',curfile), recursive = FALSE, copy.mode = TRUE)
}
...
filename1=file.path(data_dir['scenario_dev_dir_old','loc'],"precip_regressed.txt")
...
filename1=file.path(data_dir['scenario_dev_dir_old','loc'],"ref_et_monthly.txt")
